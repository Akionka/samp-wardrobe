//! Private GTA model-slot cache used for remote players.
//!
//! This cache owns only synthetic GTA model IDs. Local player instance sources
//! live in the sibling `instance_cache` module and must never enter this path.
...
pub(super) struct ModelCache {
    loaded: HashMap<String, LoadedSkin>,
...
impl ModelCache {
    ...
    pub(super) fn is_private_model(&self, model_id: i16) -> bool {
    ...
    pub(super) fn model_for(
        ...
        let now = Instant::now();
        let loaded_model = self.loaded.get(skin_id);
        ...
        match gta::load_skin(frame, skin_id, definition, recycled_model_id) {
            Ok(resources) => {
    ...
    pub(super) fn cleanup_retired(
        &mut self,
        frame: &GameFrame,
        live_model_ids: Option<HashSet<i16>>,
    ) {
